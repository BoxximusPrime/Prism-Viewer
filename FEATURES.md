...
- Sun/moon and projector PCSS contact-hardening shadows with a Graphics > PCSS tab, immediate toggle, sun/moon angular size, projector emitter size, minimum/maximum softness in metres, contact bias and three quality levels. Reuses existing maps; per-texel comparisons preserve grazing contacts and depth-aware receiver normals prevent silhouette quad leaks. Off by default; requires 32 fragment texture units; minimum softness starts at 0.02 m and can be set to zero. (in progress; uncommitted; Release build, shader startup, 125 PCSS GPU cases, 728 SSS checks, alpha/projector lighting regressions, preference layout/enabled states and seven-setting Cancel restoration passed; in-world appearance/performance verification pending)
