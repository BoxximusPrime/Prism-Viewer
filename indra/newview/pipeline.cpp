...
bool LLPipeline::allocateScreenBufferInternal(U32 resX, U32 resY)
{
    ...
    if (!gCubeSnapshot) // hack to not re-allocate various targets for cube snapshots
    {
        ...
        {
            const U32 wide_x = (resX + 3) / 4, wide_y = (resY + 3) / 4;
            if (!mSSSDiffuse.allocate(resX, resY, GL_RGBA16F) ||
                !mSSSScratch.allocate(resX, resY, GL_RGBA16F) ||
                !mSSSWide.allocate(wide_x, wide_y, GL_RGBA16F) ||
                !mSSSWide.addColorAttachment(GL_RGBA32F) ||
                !mSSSWideScratch.allocate(wide_x, wide_y, GL_RGBA16F) ||
                !mSSSWideResult.allocate(wide_x, wide_y, GL_RGBA16F))
            {
                mSSSDiffuse.release();
                mSSSScratch.release();
                mSSSWide.release();
        ...
        if (RenderFSAAType == 1 || RenderFSAAType == 2)
        {
...
void LLPipeline::releaseGLBuffers()
{
    ...
    if (mPCSSSampler)
    {
        glDeleteSamplers(1, &mPCSSSampler);
        mPCSSSampler = 0;
    }
    ...
    for (auto& light : mSSSDepthLights) light = nullptr;
    mSSSDepthLightFade = glm::vec2(0);
    mSSSDepthObjectIDs.clear();
    ...
    mUIScreen.release();
    for (auto& target : mUIBackdrop) target.release();
...
{
    resetTAAHistory(); // Agent-space transforms change at region crossings.
    mSSSDepthFocusValid = false;
    mSSSDepthFocusID.setNull();
    mSSSDepthFocusFade = 0.f;
    mSSSDepthValid = glm::vec3(0);
...
bool LLPipeline::isTAAAvailable() const
{
    return RenderFSAAType == 3 && gTAAResolveProgram.isComplete() &&
        gTAACameraProgram.isComplete() && gTAACopyProgram.isComplete() &&
...
void LLPipeline::beginTAAFrame(bool for_snapshot)
{
    ...
    mTAAProjection = get_current_projection();
    mTAAView = get_current_modelview();
    static S32 previous_camera_mode = -1;
    const S32 camera_mode = S32(gAgentCamera.getCameraMode());
    if (camera_mode != previous_camera_mode) resetTAAHistory();
    previous_camera_mode = camera_mode;
    glm::mat4 camera = glm::inverse(mTAAView), previous_camera = glm::inverse(mTAAPreviousView);
    bool projection_changed = false;
    for (S32 c = 0; c < 4; ++c) for (S32 r = 0; r < 4; ++r)
        projection_changed |= fabsf(mTAAProjection[c][r] - mTAAPreviousProjection[c][r]) > .0001f;
    if (mTAALastFrame + 1 != gFrameCount || gFrameIntervalSeconds.value() > .25f || projection_changed ||
...
void LLPipeline::endTAAFrame()
{
    if (mTAAFrameActive)
    {
        ...
        set_current_projection(mTAAProjection);
        gGL.matrixMode(LLRender::MM_PROJECTION);
        gGL.loadMatrix(glm::value_ptr(mTAAProjection));
        gGL.matrixMode(LLRender::MM_MODELVIEW);
    }
    mTAAFrameActive = false;
}

void LLPipeline::copyTAA(LLRenderTarget& src, LLRenderTarget& dst, S32 mode)
{
    LLGLDepthTest depth(GL_FALSE, GL_FALSE);
    LLGLDisable blend(GL_BLEND);
    LLGLDisable cull(GL_CULL_FACE);
    dst.bindTarget();
    auto& shader = gTAACopyProgram;
    shader.bind();
    // bindTexture expects a reserved uniform index, not a GL uniform location.
    ...
    mScreenTriangleVB->setBuffer();
    mScreenTriangleVB->drawArrays(LLRender::TRIANGLES, 0, 3);
    shader.unbindTexture(LLShaderMgr::TAA_SOURCE); shader.unbindTexture(LLShaderMgr::TAA_ORIGINAL);
    shader.unbind();
    dst.flush();
}
...
void LLPipeline::renderTAAMotion()
{
    ...
    LLGLDepthTest depth(GL_FALSE, GL_FALSE);
    gGL.setColorMask(true, true);
    // The camera pass reads scene depth; render into a color-only scratch target
    // to avoid sampling a texture attached to the active framebuffer.
    mTAAResolved.bindTarget();
    auto& camera = gTAACameraProgram;
    camera.bind();
    ...
    camera.uniform2f(LLStaticHashedString("taa_rcp_res"), 1.f / mTAAMotion.getWidth(), 1.f / mTAAMotion.getHeight());
    camera.uniform2f(LLStaticHashedString("taa_jitter"), mTAAJitter.x, mTAAJitter.y);
    camera.uniform2f(LLStaticHashedString("taa_reactive_depth_range"), 0.f, 0.f);
    mScreenTriangleVB->setBuffer();
    mScreenTriangleVB->drawArrays(LLRender::TRIANGLES, 0, 3);
    camera.unbindTexture(LLShaderMgr::DEFERRED_DEPTH);
    camera.unbind();
    // Classic avatar bodies and impostors have no previous vertex positions.
    // Conservatively reject their screen bounds; precise mesh motion below
    // replaces this mask wherever a skinned surface matches the visible depth.
    {
        LLGLEnable scissor(GL_SCISSOR_TEST);
        glColorMask(GL_FALSE, GL_FALSE, GL_FALSE, GL_TRUE);
        camera.bind();
        camera.bindTexture(LLShaderMgr::DEFERRED_DEPTH, &mRT->deferredScreen, true, LLTexUnit::TFO_POINT);
        glm::mat4 vp = get_current_projection() * mTAAView;
        for (LLCharacter* character : LLCharacter::sInstances)
        {
            LLVOAvatar* avatar = static_cast<LLVOAvatar*>(character);
            if (avatar->isDead() || avatar->isUIAvatar() || !avatar->isVisible()) continue;
            const LLVector3* bounds = avatar->getLastAnimExtents();
            glm::vec2 lo(1.f), hi(0.f);
            F32 near_depth = F32_MAX, far_depth = 0.f;
            bool crosses_near = false;
            for (S32 corner = 0; corner < 8; ++corner)
            {
                glm::vec4 p(bounds[(corner & 1) != 0].mV[0], bounds[(corner & 2) != 0].mV[1],
                            bounds[(corner & 4) != 0].mV[2], 1.f);
                F32 z = -(mTAAView * p).z;
                near_depth = llmin(near_depth, z); far_depth = llmax(far_depth, z);
                p = vp * p;
                if (p.w <= .001f) { crosses_near = true; continue; }
                glm::vec2 uv = glm::vec2(p) / p.w * .5f + .5f;
                lo = glm::min(lo, uv); hi = glm::max(hi, uv);
            }
            if (far_depth <= 0.f) continue;
            if (crosses_near) { lo = glm::vec2(0.f); hi = glm::vec2(1.f); }
            S32 x0 = llclamp(S32(floorf(lo.x * mTAAMotion.getWidth())) - 2, 0, S32(mTAAMotion.getWidth()));
            S32 y0 = llclamp(S32(floorf(lo.y * mTAAMotion.getHeight())) - 2, 0, S32(mTAAMotion.getHeight()));
            S32 x1 = llclamp(S32(ceilf(hi.x * mTAAMotion.getWidth())) + 2, 0, S32(mTAAMotion.getWidth()));
            S32 y1 = llclamp(S32(ceilf(hi.y * mTAAMotion.getHeight())) + 2, 0, S32(mTAAMotion.getHeight()));
            glScissor(x0, y0, llmax(x1 - x0, 0), llmax(y1 - y0, 0));
            camera.uniform2f(LLStaticHashedString("taa_reactive_depth_range"), llmax(near_depth - .1f, 0.f), far_depth + .1f);
            mScreenTriangleVB->setBuffer();
            mScreenTriangleVB->drawArrays(LLRender::TRIANGLES, 0, 3);
        }
        camera.unbindTexture(LLShaderMgr::DEFERRED_DEPTH);
    ...
    mTAAResolved.flush();
    copyTAA(mTAAResolved, mTAAMotion);
    mTAAMotion.bindTarget();

    // Reuse visible surface depth. Masked holes stay holes without resampling
    // their material textures; no second depth or color shading pass is needed.
    LLGLDepthTest visible(GL_TRUE, GL_FALSE, GL_EQUAL);
    std::set<LLDrawInfo*> visited;
    const U32 types[] = { LLRenderPass::PASS_SIMPLE, LLRenderPass::PASS_FULLBRIGHT,
        LLRenderPass::PASS_SHINY, LLRenderPass::PASS_FULLBRIGHT_SHINY,
        LLRenderPass::PASS_MATERIAL, LLRenderPass::PASS_MATERIAL_ALPHA_MASK, LLRenderPass::PASS_MATERIAL_ALPHA_EMISSIVE,
        LLRenderPass::PASS_SPECMAP, LLRenderPass::PASS_SPECMAP_MASK, LLRenderPass::PASS_SPECMAP_EMISSIVE,
        LLRenderPass::PASS_NORMMAP, LLRenderPass::PASS_NORMMAP_MASK, LLRenderPass::PASS_NORMMAP_EMISSIVE,
        LLRenderPass::PASS_NORMSPEC, LLRenderPass::PASS_NORMSPEC_MASK, LLRenderPass::PASS_NORMSPEC_EMISSIVE,
        LLRenderPass::PASS_ALPHA_MASK, LLRenderPass::PASS_FULLBRIGHT_ALPHA_MASK,
        LLRenderPass::PASS_GLTF_PBR, LLRenderPass::PASS_GLTF_PBR_ALPHA_MASK };
    for (U32 rigged = 0; rigged < 2; ++rigged)
    {
        auto& shader = gTAAMotionProgram[rigged];
        shader.bind();
        shader.uniform2f(LLStaticHashedString("taa_rcp_res"), 1.f / mTAAMotion.getWidth(), 1.f / mTAAMotion.getHeight());
        shader.uniform2f(LLStaticHashedString("taa_jitter"), mTAAJitter.x, mTAAJitter.y);
        for (U32 type : types)
        {
            auto* end = endRenderMap(type + rigged);
            for (auto* it = beginRenderMap(type + rigged); it != end; )
            {
                LLDrawInfo* info = *it;
...
LLRenderTarget* LLPipeline::resolveTAA()
{
    ...
    shader.uniform1f(LLStaticHashedString("taa_history_weight"), llclamp(gSavedSettings.getF32("RenderTAAHistoryWeight"), .5f, .97f));
    shader.uniform1f(LLStaticHashedString("taa_motion_protection"), llclamp(gSavedSettings.getF32("RenderTAAMotionProtection"), 0.f, 1.f));
    ...
    shader.unbind();
    history.flush();
    copyTAA(history, mTAAResolved, 1);
    mTAAIndex = 1 - mTAAIndex;
...
void LLPipeline::renderTAADebug(LLRenderTarget& dst)
{
...
void LLPipeline::renderFinalize()
{
    ...
    LLRenderTarget* antialiased = resolveTAA();
    static LLCachedControl<bool> has_hdr(gSavedSettings, "RenderHDREnabled", true);
    bool hdr = gGLManager.mGLVersion > 4.05f && has_hdr();
    if (hdr)
    {
    ...
    {
        gammaCorrect(antialiased, &mPostPingMap);
    }
...
void LLPipeline::bindShadowMaps(LLGLSLShader& shader)
{
    ...
    static LLCachedControl<F32> pcss_projector_size(gSavedSettings, "RenderPCSSProjectorSize", 0.1f);
    static LLCachedControl<F32> pcss_bias(gSavedSettings, "RenderPCSSBias", 5.f);
    static LLCachedControl<S32> pcss_quality(gSavedSettings, "RenderPCSSQuality", 1);
    ...
    shader.uniform4f(params, pcss ? tanf(llclamp(F32(pcss_angle), 0.1f, 10.f) * DEG_TO_RAD * 0.5f) : 0.f,
        max_radius, llclamp(F32(pcss_bias), 0.f, 30.f) * 0.001f,
        llclamp(F32(pcss_minimum), 0.f, llmin(max_radius, 0.5f)));
    ...
    for (U32 i = 0; i < 6; i++)
    {
        LLRenderTarget* shadow_target = i < 4 ? getSunShadowTarget(i) : getSpotShadowTarget(i - 4);
        if (shadow_target)
        {
            ...
            channel = shader.enableTexture(LLShaderMgr::PCSS_DEPTH0 + i, LLTexUnit::TT_TEXTURE);
            if (channel > -1)
            {
                if (!mPCSSSampler)
                {
                    glGenSamplers(1, &mPCSSSampler);
                    glSamplerParameteri(mPCSSSampler, GL_TEXTURE_COMPARE_MODE, GL_NONE);
                    glSamplerParameteri(mPCSSSampler, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
                    glSamplerParameteri(mPCSSSampler, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
                    glSamplerParameteri(mPCSSSampler, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
                    glSamplerParameteri(mPCSSSampler, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
                }
                // The same depth texture keeps hardware PCF/SSS comparison
                // sampling on its ordinary unit; only this unit reads raw depth.
                gGL.getTexUnit(channel)->bind(shadow_target, true);
                shader.bindSampler(LLShaderMgr::PCSS_DEPTH0 + i, mPCSSSampler);
            }
        }
    }
...
void LLPipeline::bindDeferredShader(LLGLSLShader& shader, LLRenderTarget* light_target, LLRenderTarget* depth_target)
{
    ...
    shader.uniform1f(sss_penetration_name, llclamp(F32(sss_penetration), 5.f, 300.f) * 0.001f);
    ...
    // Accumulate different AO samples only when the main image has a TAA resolve.
    shader.uniform1i(gtaoNoise, mTAAFrameActive && !gCubeSnapshot && !sImpostorRender && mRT == &mMainRT ? S32(mTAASequence % 64) : 0);
...
{
    glm::mat4 matrix, agentMatrix, agentView;
...
ProjectorParams getProjectorParams(LLDrawable* drawablep)
{
    ...
    const glm::mat4 agent_view = glm::inverse(light_to_agent);
...
{
    LLVOVolume* volume = drawablep->getVOVolume();
    const bool no_shadow = volume->projectorShadowsDisabled();
    ...
    for (U32 i = 0; i < 2; i++)
    {
        if (!no_shadow && mShadowSpotLight[i] == drawablep)
        {
            s_idx = i;
        }
    }
...
void LLPipeline::renderShadow(const glm::mat4& view, const glm::mat4& proj, LLCamera& shadow_cam, LLCullResult& result, bool depth_clamp)
{
    ...
    {
        // Entry and exit have the same frustum. Reuse its sorted geometry.
        grabReferences(result);
    }
...
namespace
{
// A 200 ms transition, with bounded progress after a stalled frame.
F32 advanceSSSDepthFade(F32 fade, bool retained, F32 dt)
{
    return llclamp(fade + (retained ? 5.f : -5.f) * llclamp(dt, 0.f, 0.1f), 0.f, 1.f);
}

bool sssProjectorIntersectsSphere(const glm::mat4& projection, const glm::vec3& center, F32 radius)
{
    // Projector texture coordinates occupy [0,w], including the near/far planes.
    const glm::mat4 rows = glm::transpose(projection);
    for (U32 axis = 0; axis < 3; ++axis)
    {
        for (const glm::vec4& plane : { rows[axis], rows[3] - rows[axis] })
        {
            if (glm::dot(plane, glm::vec4(center, 1.f)) < -radius * glm::length(glm::vec3(plane)))
                return false;
        }
    }
    return true;
...
F32 sssLightScore(const LLColor3& color, F32 distance, F32 radius, F32 falloff)
{
    if (radius <= 0.001f) return 0.f;
    // Match calcLegacyDistanceAttenuation; color is already linear and includes intensity.
    const F32 attenuation = 1.f - llclamp((distance / radius + falloff) / (1.f + falloff), 0.f, 1.f);
...
{
    LLUUID selected;
    LLVector3 selected_position, retained_position = mSSSDepthFocus;
    F32 nearest = F32_MAX;
    const F32 range = llclamp(gSavedSettings.getF32("BoxxySSSMaxDistance"), 1.f, 32.f);
    if (mHasSSSGeometry && sCull && gSavedSettings.getBOOL("BoxxySSSShadowThickness") &&
        gSavedSettings.getS32("BoxxySSSMode") == 2 && gSavedSettings.getF32("BoxxySSSTransmission") > 0.f)
    {
        for (U32 type = 0; type < LLRenderPass::NUM_RENDER_TYPES; ++type)
        {
            ...
            for (auto* i = beginRenderMap(type); i != end; LLCullResult::increment_iterator(i, end))
            {
                ...
                const LLViewerObject* object = info->mSSSObject;
                if (!object || object->isDead() || info->mFullbright || info->mSSSFrameTag != mSSSFrameTag) continue;
                ...
                const bool retained = object->getID() == mSSSDepthFocusID;
    ...
    const bool retained = selected.notNull() && selected == mSSSDepthFocusID;
    mSSSDepthFocusFade = advanceSSSDepthFade(mSSSDepthFocusFade, retained, gFrameIntervalSeconds.value());
    mSSSDepthFocus = retained ? selected_position : retained_position;
    // Finish fading the old subject before changing the projection.
    if (mSSSDepthFocusFade == 0.f)
    {
        mSSSDepthFocusID = selected;
        if (selected.notNull()) mSSSDepthFocus = selected_position;
    }
    mSSSDepthFocusValid = mSSSDepthFocusID.notNull() && mSSSDepthFocusFade > 0.f;
}

void LLPipeline::setSSSDepthUniforms(LLGLSLShader& shader, const LLViewerObject* object)
{
    static const LLStaticHashedString pass("sss_depth_pass"), identity("sss_depth_id");
    F32 id = 0.f; // Unclassified geometry always blocks transmission.
    if (mSSSDepthPass != 0 && mSSSDepthOpaque && object)
    {
        // IDs are shared by entry/exit draws for this frame and exactly representable in RGBA32F.
        const auto result = mSSSDepthObjectIDs.emplace(object, U32(mSSSDepthObjectIDs.size()) + 1);
        if (result.first->second < 16777216) id = F32(result.first->second);
    }
    shader.uniform1i(pass, mSSSDepthPass);
    shader.uniform1f(identity, id);
}
...
void LLPipeline::bindSSSDepth(LLGLSLShader& shader)
{
    ...
    for (U32 i = 0; i < 3; ++i)
    {
        ...
        used = true;
        if (mSSSDepth[i].getNumTextures() == 0 && !mSSSDepth[i].allocate(1, 1, GL_RGBA32F))
        {
            mSSSDepth[i].release();
            mSSSDepthValid[i] = 0.f;
            gGL.getTexUnit(channel)->bind(LLViewerFetchedTexture::sBlackImagep);
            continue;
        }
        gGL.getTexUnit(channel)->bind(&mSSSDepth[i], false);
        gGL.getTexUnit(channel)->setTextureFilteringOption(LLTexUnit::TFO_POINT);
        gGL.getTexUnit(channel)->setTextureAddressMode(LLTexUnit::TAM_CLAMP);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_COMPARE_MODE, GL_NONE);
    }
...
void LLPipeline::generateSSSDepth(LLCamera& camera)
{
    ...
    for (const auto& light : mNearbyLights)
    {
        ...
        if (drawable == mSSSDepthLights[0] || drawable == mSSSDepthLights[1]) score *= 1.25f;
        if (score > scores[0])
        {
    ...
    for (U32 i = 0; i < 2; ++i)
    {
        if (mSSSDepthLights[i].notNull() && mSSSDepthLights[i]->isDead())
        {
            mSSSDepthLights[i] = nullptr;
            mSSSDepthLightFade[i] = 0.f;
        }
        const bool retained = mSSSDepthLights[i].notNull() &&
            (mSSSDepthLights[i] == desired[0] || mSSSDepthLights[i] == desired[1]);
        mSSSDepthLightFade[i] = advanceSSSDepthFade(mSSSDepthLightFade[i], retained, gFrameIntervalSeconds.value());
        if (mSSSDepthLightFade[i] == 0.f && !retained)
        {
            mSSSDepthLights[i] = nullptr;
            for (LLDrawable* candidate : desired)
            {
                if (candidate && candidate != mSSSDepthLights[1 - i])
                {
                    mSSSDepthLights[i] = candidate;
                    break;
                }
            }
        }
        lights[i] = mSSSDepthLights[i];
    }
    mSSSDepthObjectIDs.clear();
    ...
    for (U32 i = 0; i < 3; ++i)
    {
        ...
        if (i > 0 && (!lights[i - 1] || mSSSDepthLightFade[i - 1] <= 0.f)) continue;
        glm::vec3 origin;
        glm::mat4 proj, view;
        ...
        else
        {
            ...
            // A close projector may illuminate skin behind the center-facing camera.
            ...
            // Spend depth precision on the subject and the longest allowed tissue path.
        ...
        {
            target.release();
            if (!target.allocate(resolution, resolution, GL_RGBA32F, true))
            {
                target.release();
                continue;
            }
        ...
        {
            LL_PROFILE_ZONE_NAMED("SSS depth target clear");
            target.bindTarget();
            target.getViewport(gGLViewport);
            gGL.setColorMask(true, true);
            glClearColor(1.f, 0.f, 1.f, 0.f); // Missing entry/exit: no skin identity.
            target.clear();
        }
        ...
        {
            mSSSDepthPass = pass;
            glCullFace(pass == 1 ? GL_BACK : GL_FRONT);
            gGL.setColorMask(pass == 1, pass == 1, pass == 2, pass == 2);
            if (pass == 1)
            {
                LL_PROFILE_ZONE_NAMED("SSS depth entry");
                renderShadow(view, proj, shadowCamera, results[i], true);
            }
            else
            {
                {
                    LL_PROFILE_ZONE_NAMED("SSS depth exit clear");
                    target.clear(GL_DEPTH_BUFFER_BIT);
                }
                LL_PROFILE_ZONE_NAMED("SSS depth exit");
                renderShadow(view, proj, shadowCamera, results[i], true);
            }
        }
        mSSSDepthPass = 0;
        RenderSpotLight = nullptr;
        {
            LL_PROFILE_ZONE_NAMED("SSS depth target flush");
            target.flush();
        }
        ...
        mSSSDepthValid[i] = mSSSDepthFocusFade * (i == 0 ? 1.f : mSSSDepthLightFade[i - 1]);
    }
...
void LLPipeline::generateSunShadow(LLCamera& camera)
{
    ...
    if (gen_shadow)
    {
        // A newly tagged source must release both current and pending slots,
        // without the normal fade-out retaining a shadow or starving other lights.
        for (U32 i = 0; i < 2; ++i)
        {
            auto shadow_disabled = [](LLDrawable* light)
            {
                LLVOVolume* volume = light ? light->getVOVolume() : nullptr;
                return light && (!volume || volume->projectorShadowsDisabled());
            };
            if (shadow_disabled(mTargetShadowSpotLight[i])) mTargetShadowSpotLight[i] = nullptr;
            if (shadow_disabled(mShadowSpotLight[i]))
            {
