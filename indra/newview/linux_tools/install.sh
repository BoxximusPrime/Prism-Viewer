...
# Install the Prism Viewer. This script can install the viewer both
...
function homedir_install()
{
    ...
    warn "to install the Prism Viewer in your home directory. If you"
    warn "would like to install the Prism Viewer system-wide, please run"
    ...
    install_to_prefix "$HOME/.prism-install"
    $HOME/.prism-install/etc/refresh_desktop_app_entry.sh
}

function root_install()
{
    local default_prefix="/opt/prism-install"
