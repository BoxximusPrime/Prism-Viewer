...
float pcssCompare(sampler2D depthMap, vec2 uv, vec3 receiver, vec2 slope, float bias)
{
                 ...
                 vec3 normal, vec3 lightDir, float sourceRadius)
{
    ...
    for (int i = 0; i < searchCount; ++i)
    {
        ...
        if (depth < receiverDepth + bias && depth < 1.0)
        {
    ...
    if (radius <= 0.0) return pcssCompare(depthMap, tc.xy, tc, slope, bias);
    float shadow = 0.0;
    for (int i = 0; i < filterCount; ++i)
    {
        ...
        else shadow += pcssCompare(depthMap, uv, tc, slope, bias);
    }
