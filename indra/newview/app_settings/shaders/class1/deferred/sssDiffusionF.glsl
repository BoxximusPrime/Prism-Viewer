...
void main()
{
    ...
    float radius = clamp(sss_depth * 0.5 * screen_res.y /
                         max(abs(inv_proj[1][1]) * -pos.z, 0.001), 0.0, sss_smoothing_pass != 0 ? 64.0 : 24.0);
    ...
    vec3 sigma = sss_smoothing_pass != 0 ? vec3(0.8) :
        mix(vec3(0.65), vec3(1.0, 0.55, 0.35), sss_params.z);
    vec3 sum = vec3(0.0);
    vec3 weights = vec3(0.0);
    // A wide transmission filter needs contiguous pixel samples. Keeping only
    // 13 taps at a 64px radius creates separated copies of thin finger edges.
    int taps = sss_smoothing_pass != 0 ? max(int(ceil(radius)), 1) : 6;
    for (int i = -taps; i <= taps; ++i)
    {
        float offset = float(i) / float(taps);
        ...
        if (sss_smoothing_pass != 0 && separation > max(sss_depth, 0.002)) continue;
