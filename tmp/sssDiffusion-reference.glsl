...
void main()
{
    ...
    float radius = max(sss_depth * 0.5 * screen_res.y /
                       max(abs(inv_proj[1][1]) * -pos.z, 0.001), 0.0);
    ...
    // Keep the requested meter radius even in close-ups. Sample contiguous
    // pixels so widening the filter cannot make copies of thin bright features.
    // Bound work to the visible row/column, not the scattering distance.
