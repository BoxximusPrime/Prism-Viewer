...
def run(sdl, gl):
    ...
    for angle in (5,15,25,35,45,55,65,75,85,89.9):
        ...
        for fbo,projection in ((shadow_fbo,clip_light),(camera_fbo,proj)):
            ...
            gl.ClearDepth(.5-2/depth_range if fbo==shadow_fbo else 1)
            gl.Clear(0x4100)
            matrix(capture,'projection',projection)
            if fbo != shadow_fbo: gl.DrawArrays(4,0,len(points)//3)
        ...
        for prog, mode in ((lighting,'deferred'),(forward,'forward')):
            ...
            count=sum(v>.01 for v in visible)
            print(angle, mode, 'wall leaks', count, '/', len(visible), 'peak', max(visible), flush=True)
            cases+=1
