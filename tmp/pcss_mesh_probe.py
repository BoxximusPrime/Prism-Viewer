"""Rasterized mesh probe for PCSS receiver-plane artifacts (stdlib + SDL3)."""
import ctypes as C
import math
...
def run(sdl, gl):
    ...
    def program(vertex, fragment, helpers=False):
        ...
        if helpers:
            ...
            if '--before' in sys.argv:
                sources[2] = (0x8B30, (ROOT/'tmp/pcss-before-backface-fix/shadowUtil.glsl').read_text())
                sources[3] = (0x8B30, (ROOT/'tmp/pcss-before-backface-fix/pcssUtil.glsl').read_text())
    ...
    for scale, distance in ((1,3.0),(1,1.3),(.01,.03)):
        ...
        lx,lz=.8,.6
        extent, depth_range=4*scale,32*scale
        ...
        for prog, mode in ((lighting,'deferred'),(forward,'forward')):
            ...
            bias = float(next((a.split('=')[1] for a in sys.argv if a.startswith('--bias=')), '.005'))
            uniform(prog,'pcss_params',math.tan(math.radians(.53)*.5),scale,bias*scale,0)
            ...
            valid=[output[i] for i in range(0,len(output),4) if output[i+3]>.5 and output[i+1]>.2]
            dark=sum(v<.99 for v in valid)
            print(distance,mode,'front pixels',len(valid),'shadowed',dark,'min',min(valid),flush=True)
            print('away from camera',sum(output[i+3]>.5 and output[i+2]<0 for i in range(0,len(output),4)),flush=True)
            for lo, hi in ((-1,-.1),(-.1,0),(0,.1),(.1,.2)):
                band=[output[i] for i in range(0,len(output),4) if output[i+3]>.5 and lo <= output[i+1] < hi]
                if band: print('BAND',lo,hi,'count',len(band),'min/max',min(band),max(band),'lit',sum(v>.1 for v in band),flush=True)
            pixels=[]
            for i in range(0,len(output),4):
                s,n,_,alpha=output[i:i+4]
                v=int(max(0,min(1,.15+.85*s))*255) if alpha else 0
                pixels.extend([v]*3)
            png(ROOT/'tmp'/f'pcss-mesh-{distance}-{mode}{"-before" if "--before" in sys.argv else ""}.png',pixels)
